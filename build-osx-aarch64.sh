...
APPBASE="build/macos-aarch64/BaseScape.app"
...
build() {
    ...
    cp native/build-aarch64/src/BaseScape $APPBASE/Contents/MacOS/
    cp target/BaseScape.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on BaseScape
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/BaseScape
    popd

    otool -l $APPBASE/Contents/MacOS/BaseScape
}
...
dmg() {
    ...
    mv BaseScape\ *.dmg BaseScape-aarch64.dmg

    # dump for CI
    hdiutil imageinfo BaseScape-aarch64.dmg

    if ! hdiutil imageinfo BaseScape-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo BaseScape-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit BaseScape-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple BaseScape-aarch64.dmg
    fi
}
